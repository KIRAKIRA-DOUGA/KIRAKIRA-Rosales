...
const urlHeartbeatDatabaseShardData2Array = (heartbeatDatabaseShardDataList: string[]): serviceInfoType[] => {
	...
	// FIXME
	console.log('heartbeatDatabaseShardDataList', heartbeatDatabaseShardDataList) // FIXME
	// FIXME
	heartbeatDatabaseShardDataList.forEach((heartbeatDatabaseShardData: string) => {
		...
		// FIXME
		console.log('heartbeatDatabaseShardDataArray', heartbeatDatabaseShardDataArray) // FIXME
		// FIXME
	...
	return new Promise<serviceInitState>((resolve, reject) => {
		...
		if (adminOneTimeSecretKeyCheckResult.state) { // 如果一次性密钥正确，则继续
			...
			if (allInitEnvRight) { // 如果验证通过
				const { systemAdminUserName, systemAdminPasswordBase64, localhostServicePublicIPAddress, localhostServicePrivateIPAddress, localhostServicePort, heartbeatDatabaseShardData } = initEnvs // 玩的就是解构，不解构你就不酷了
				...
				// FIXME
				console.log('__HEARTBEAT_DB_SHARD_LIST__', ctx.state.__HEARTBEAT_DB_SHARD_LIST__) // FIXME
				// FIXME
				if (ctx.state.__HEARTBEAT_DB_SHARD_LIST__) {
					createOrMergeHeartBeatDatabaseConnectByDatabaseInfo(ctx.state.__HEARTBEAT_DB_SHARD_LIST__, ctx).then(async (connects: mongoDBConnectType[]) => { // 连接到心跳数据库，回调结果是数据库连接(asPromise)
						...
						// FIXME
						console.log('__HEARTBEAT_DB_SHARD_CONNECT_LIST__', ctx.state.__HEARTBEAT_DB_SHARD_CONNECT_LIST__) // FIXME
						// FIXME
						...
						const administratorData: adminUserType[] = [{ userName: systemAdminUserName, password: systemAdminPasswordBase64 }] // 数据
